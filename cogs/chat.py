...
from tools.checks import Messages, Perms
...
class Chat(commands.Cog): 
  ...
  async def on_autoresponder(self, message: discord.Message): 
   if Messages.good_message(message): 
    res = await self._db_fetchrow("SELECT response FROM autoresponder WHERE guild_id = $1 AND trigger = $2", message.guild.id, message.content)
    if res:
     retry_after = self.get_ratelimit(message)
     if retry_after: return
     if '{delete}' in res['response']: await message.delete()
  ...
  async def on_autoreact(self, message: discord.Message): 
   if Messages.good_message(message): 
    check = await self._db_fetchrow("SELECT emojis FROM autoreact WHERE guild_id = $1 AND trigger = $2", message.guild.id, message.content)
    if check: 
     retry_after = self.get_ratelimit(message)
     if retry_after: return
  ...
  @autoresponder.command(name="add", help="config", description="add an autoresponder", brief="manage guild", usage='[trigger] [response]\nexample: ;ar add hello world, the world is amazing')  
  @commands.has_guild_permissions(manage_guild=True)
  async def ar_add(self, ctx: commands.Context, *, arg: str): 
   arg = arg.split(', ')
   trigger = arg[0]
   try: response = arg[1]
   except: return await ctx.send_warning("No response found")
   ...
   if check: 
    ...
    return await ctx.send_success("Updated autoresponder with trigger `{}` and response `{}`".format(trigger, response))
   else: 
  ...
  @autoresponder.command(name="remove", help="config", description="remove an autoresponder", brief="manage guild", usage="[trigger]")
  @commands.has_guild_permissions(manage_guild=True)
  async def ar_remove(self, ctx: commands.Context, *, trigger: str): 
    check = await self.bot.db.fetchrow("SELECT * FROM autoresponder WHERE guild_id = $1 AND trigger = $2", ctx.guild.id, trigger)
    if not check: return await ctx.send_warning(f"There is no autoresponder with the trigger **{trigger}**") 
    await self.bot.db.execute('DELETE FROM autoresponder WHERE guild_id = $1 AND trigger = $2', ctx.guild.id, trigger)
    await ctx.send_success(f"Deleted autoresponder with the trigger **{trigger}**")
  ...
  async def ar_list(self, ctx: commands.Context): 
   ...
   for result in results: 
    ...
    if l == 10: 
      embeds.append(discord.Embed(color=self.bot.color, title=f"auto responders ({len(results)})", description=mes))
      l=0
   embeds.append(discord.Embed(color=self.bot.color, title=f"auto responders ({len(results)})", description=mes)) 
   await ctx.paginator(embeds)

  @commands.group(invoke_without_command=True, aliases=['rt'])
  ...
  async def autoreact_add(self, ctx: commands.Context, *, content: str):
   ...
   await ctx.send_success(f"Added autoreact for **{con[0]}** - {''.join([e for e in emojis])}")
  ...
  async def autoreact_remove(self, ctx: commands.Context, *, content: str): 
   ...
   if not check: return await ctx.send_warning(f"No autoreaction found with the content **{content}**")
   await self.bot.db.execute("DELETE FROM autoreact WHERE guild_id = $1 AND trigger = $2", ctx.guild.id, content)
   return await ctx.send_success(f"Deleted autoreaction with the content **{content}**")
