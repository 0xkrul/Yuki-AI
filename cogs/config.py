...
{user.discriminator} - returns the user's discriminator
 ...
 {guild.boost_tier} - returns the server's boost level
   """
    ...
    @dm.command(name="jail", description="custom jail direct message", help="config", brief="manage guild", usage="[message | embed]\nexample: dm jail --embed jail")
    @utils.get_perms("manage_guild")
    async def dm_jail(self, ctx: Context, *, code: str):
      await dm_cmds(ctx, code)

    @dm.command(name="warn", description="custom warn direct message", help="config", brief="manage guild", usage="[message | embed]\nexample: dm warn --embed warn")
    @utils.get_perms("manage_guild")
    async def dm_warn(self, ctx: Context, *, code: str):
      await dm_cmds(ctx, code)  
    
    @dm.command(name="mute", description="custom mute direct message", help="config", brief="manage guild", usage="[message | embed]\nexample: dm mute --embed mute")
    @utils.get_perms("manage_guild")
    async def dm_mute(self, ctx: Context, *, code: str):
      await dm_cmds(ctx, code)
    
    @dm.command(name="ban", description="custom ban direct message", help="config", brief="manage guild", usage="[message | embed]\nexample: dm ban --embed ban")
    @utils.get_perms("manage_guild")
    async def dm_ban(self, ctx: Context, *, code: str):
      await dm_cmds(ctx, code)

    @dm.command(name="kick", description="custom kick direct message", help="config", brief="manage guild", usage="[message | embed]\nexample: dm kick --embed kick")
    @utils.get_perms("manage_guild")
    ...
    async def confessions_mute(self, ctx: Context, *, number: int): 
     ...
     if check is None: return await ctx.send_warning("Confessions aren't **enabled** in this server") 
     re = await self.bot.db.fetchrow("SELECT * FROM confess_members WHERE guild_id = $1 AND confession = $2", ctx.guild.id, number)
     if re is None: return await ctx.send_warning("Couldn't find that confession")
     ...
     if r: return await ctx.send_warning("This **member** is **already** confession muted")
    ...
    async def connfessions_unmute(self, ctx: Context, *, number: str): 
      ...
      member_id = re['user_id']
      r = await self.bot.db.fetchrow("SELECT * FROM confess_mute WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, member_id)
      if not r: return await ctx.send_warning("This **member** is **not** confession muted")
    ...
    async def confessions_add(self, ctx: Context, *, channel: TextChannel): 
       ...
       return await ctx.send_success(f"confession channel set to {channel.mention}".capitalize())
    ...
    async def prefix(self, ctx: Context, prefix: str):      
       ...
       # Invalidate cache
       from entry import prefix_cache
       prefix_cache.invalidate_guild(ctx.guild.id)
    ...
    async def selfprefix(self, ctx: Context, prefix: str):      
      ...
      if prefix.lower() == "none": 
        ...
        if check is not None:
          ...
          # Invalidate cache
          from entry import prefix_cache
          prefix_cache.invalidate_self(ctx.author.id)
