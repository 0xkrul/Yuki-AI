...
unlockemoji = "<:unlock:1436068468997754881>"
lockemoji = "<:lock:1436068454800031925>"
plusemoji = "<:plus:1436068530331324506>"
minusemoji = "<:minus:1436068545925611550>"
channelemoji = "<:channel:1436068840714010768>"
unghostemoji = "<:unghost:1436075817355706510>"
ghostemoji = "<:ghost:1436068570055573717>"
claimemoji = "<:crown:1436068514942288026>"
hammeremoji = "<:hammer:1436068492599361536>"
manemoji = "<:man:1436068557783040192>" 
...
class vmbuttons(discord.ui.View):
    ...
    async def reveal(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if check is not None:     
             ...
             category = voicechannel.category 
             if await check_vc(interaction, category) is False: return 
             if not await interaction.client.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", interaction.guild.id) and not await interaction.client.db.fetchrow("SELECT * FROM donor WHERE user_id = $1", interaction.user.id) and not interaction.guild.id in interaction.client.main_guilds: return await interaction.client.ext.send_warning(interaction, "This server wasn't **upgraded** to **premium**. Join [here](https://discord.gg/popular) to upgrade!", ephemeral=True)
             che = await interaction.client.db.fetchrow("SELECT * FROM vcs WHERE voice = $1 AND user_id = $2", interaction.user.voice.channel.id, interaction.user.id)
             if che is None:
                return await interaction.client.ext.send_warning(interaction, "you don't own this voice channel".capitalize(), ephemeral=True)
             elif che is not None:
    ...
    async def hide(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if check is not None:     
             ...
             category = voicechannel.category 
             if await check_vc(interaction, category) is False: return 
             if not await interaction.client.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", interaction.guild.id) and not await interaction.client.db.fetchrow("SELECT * FROM donor WHERE user_id = $1", interaction.user.id) and not interaction.guild.id in interaction.client.main_guilds: return await interaction.client.ext.send_warning(interaction, "This server requires premium. Join [here](https://https://discord.gg/ZTTXMkk8ua) to upgrade.", ephemeral=True)
             che = await interaction.client.db.fetchrow("SELECT * FROM vcs WHERE voice = $1 AND user_id = $2", interaction.user.voice.channel.id, interaction.user.id)
             if che is None:
                return await interaction.client.ext.send_warning(interaction, "you don't own this voice channel".capitalize(), ephemeral=True)
             elif che is not None:
    ...
    async def claim(self, interaction: discord.Interaction, button: discord.ui.Button):
         ...
         if check is not None:     
             ...
             if member in interaction.user.voice.channel.members: return await interaction.client.ext.send_warning(interaction, "The owner is still in the voice channel", ephemeral=True)
             else:
    ...
    @discord.ui.button(label="", emoji=hammeremoji, style=discord.ButtonStyle.gray, custom_id="persistent_view:activity")
    async def activity(self, interaction: discord.Interaction, button: discord.ui.Button):
         ...
         if check is not None:     
            ...
            if not await interaction.client.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", interaction.guild.id) and not interaction.guild.id in interaction.client.main_guilds: return await interaction.client.ext.send_warning(interaction, "This server requires premium. Join [here](https://https://discord.gg/ZTTXMkk8ua) to upgrade.", ephemeral=True) 
            ...
            em = discord.Embed(color=interaction.client.color, title="VoiceMaster Moderation Menu", description="Moderate your voice channel using the menu below")
            options = [ 
               discord.SelectOption(
                  label="mute",
                  description="mute member in the voice channel",
                  emoji="<:muted:1035858405212041316>"
               ),
               discord.SelectOption(
                  label="unmute",
                  description="unmute members in the voice channel",
                  emoji="<:unmuted:1035864651130490970>"
               ),
               discord.SelectOption(
                  label="deafen",
                  description="deafen members in your voice channel",
                  emoji="<:deafened:1005611740874158090>"
               ),
               discord.SelectOption(
                  label="undeafen",
                  emoji="<:undeafened:1035864650169987153>",
                  description="undeafen members in your voice channel"
               ),
               discord.SelectOption(
                  label="kick",
                  description="kick members from your voice channel",
                  emoji="<:hammer:1035859123713101825>"
               )
            ]
...
class voicemaster(commands.Cog):
   ...
   def create_interface(self, ctx: commands.Context) -> discord.Embed: 
     em = discord.Embed(color=self.bot.color, title="VoiceMaster Interface", description="Use the buttons below to control your voice channel.")    
     em.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon)
     em.add_field(name="Manage", value=f"{lockemoji} [`lock`](https://https://discord.gg/ZTTXMkk8ua) the voice channel\n{unlockemoji} [`unlock`](https://https://discord.gg/ZTTXMkk8ua) the voice channel\n{ghostemoji} [`hide`](https://https://discord.gg/ZTTXMkk8ua) the voice channel\n{unghostemoji} [`reveal`](https://https://discord.gg/ZTTXMkk8ua) the voice channel\n{channelemoji} [`rename`](https://https://discord.gg/ZTTXMkk8ua) the voice channel")
     em.add_field(name="Misc", value=f"{plusemoji} [`increase`](https://https://discord.gg/ZTTXMkk8ua) the user limit\n{minusemoji} [`decrease`](https://https://discord.gg/ZTTXMkk8ua) the user limit\n{claimemoji} [`claim`](https://https://discord.gg/ZTTXMkk8ua) the voice channel\n{manemoji} [`info`](https://https://discord.gg/ZTTXMkk8ua) of the channel\n{hammeremoji} [`moderate`](https://https://discord.gg/ZTTXMkk8ua) the voice channel")    
     return em
   ...
   async def get_channel_categories(self, channel: discord.VoiceChannel, member: discord.Member) -> bool: 
     if len(channel.category.channels) == 50: 
      ...
      try: 
        ...
        await member.send("I couldn't make a new voice channel (category full of channels)", view=view)
      except: pass 
     ...
     if check:
      ...
      if not before.channel and after.channel: 
       if after.channel.id == jtc: 
        ...
        channel = await member.guild.create_voice_channel(name=f"{member.name}'s lounge", category=after.channel.category, reason="creating temporary voice channel")
       ...
       if before.channel.category == after.channel.category: 
        if after.channel.id == jtc: 
         ...
         if await self.get_channel_categories(after.channel, member) is True: return
         cha = await member.guild.create_voice_channel(name=f"{member.name}'s lounge", category=after.channel.category, reason="creating temporary voice channel")
   ...
   @voice.command(description="lock the voice channel", help="config", brief="vc owner")
   @check_vc_owner()
   ...
   @voice.command(description="unlock the voice channel", help="config", brief="vc owner")
   @check_vc_owner()
   ...
   @voice.command(description="rename the voice channel", help="config", usage="[name]", brief="vc owner")
   @check_vc_owner()
   ...
   @voice.command(description="hide the voice channel", help="config", brief="vc owner")
   @check_vc_owner()
   ...
   @voice.command(description="reveal the voice channel", help="config", brief="vc owner")
   @check_vc_owner()
   ...
   @voice.command(description="let someone join your locked voice channel", help="config", usage="[member]", brief="vc owner")
   @check_vc_owner()
   async def permit(self, ctx: commands.Context, *, member: discord.Member): 
      ...
      return await ctx.send_success( f"**{member}** is allowed to join <#{ctx.author.voice.channel.id}>")
   
   @voice.command(description="restrict someone from joining your voice channel", help="config", usage="[member]", brief="vc owner")
   @check_vc_owner()
   async def reject(self, ctx: commands.Context, *, member: discord.Member):  
      if member.id == ctx.author.id: return await ctx.reply("why would u wanna kick urself >_<")  
      ...
      return await ctx.send_success( f"**{member}** not is allowed to join <#{ctx.author.voice.channel.id}> anymore")
   
   @voice.command(name="kick", description="kick a member from your voice channel", help="config", usasge="[member]", brief="vc owner")
   @check_vc_owner()
   async def vc_kick(self, ctx: commands.Context, *, member: discord.Member): 
     if member.id == ctx.author.id: return await ctx.reply("why would u wanna kick urself >_<")  
     if not member in ctx.author.voice.channel.members: return await ctx.send_error(f"**{member}** isn't in **your** voice channel") 
     await member.move_to(channel=None)
     return await ctx.send_success( f"**{member}** got kicked from <#{ctx.author.voice.channel.id}>")
   
   @voice.command(description="claim the voice channel ownership", help="config")
   async def claim(self, ctx: commands.Context): 
    ...
    if not check: return await ctx.send_warning("You are **not** in a voice channel made by the bot")
    if ctx.author.id == check[0]: return await ctx.send_warning("You are the **owner** of this voice channel")
    if check[0] in [m.id for m in ctx.author.voice.channel.members]: return await ctx.send_warning("The owner is still in the voice channel")
   ...
   @voice.command(description="transfer the voice channel ownership to another member", help="config", usage="[member]", brief="vc owner")
   @check_vc_owner()
   async def transfer(self, ctx: commands.Context, *, member: discord.Member): 
     if not member in ctx.author.voice.channel.members: return await ctx.send_warning(f"**{member}** is not in your voice channel")
     if member == ctx.author: return await ctx.send_warning("You are already the **owner** of this **voice channel**")
   ...
   @commands.command(description="sends an updated interface of voicemaster", help="config", brief="administrator")
   ...
   async def interface(self, ctx: commands.Context): 
      ...
      if check is None: return await ctx.send_warning( "VoiceMaster isn't configured")      
   ...
   @voicemaster.command(description="sets voicemaster module for your server", help="config", brief="administrator")
   @Perms.get_perms("administrator")
   async def setup(self, ctx: commands.Context):
      check = await self.bot.db.fetchrow("SELECT * FROM voicemaster WHERE guild_id = $1", ctx.guild.id)
      if check is not None: return await ctx.send_warning( "VoiceMaster is configured") 
      elif check is None:                   
   ...
   @voicemaster.command(description="remove voicemaster module from your server", help="config", aliases=["unset"], brief="administrator")
   @Perms.get_perms("administrator")
   async def remove(self, ctx: commands.Context):
         check = await self.bot.db.fetchrow("SELECT * FROM voicemaster WHERE guild_id = $1", ctx.guild.id)
         if check is None: return await ctx.send_warning( "VoiceMaster is not configured") 
