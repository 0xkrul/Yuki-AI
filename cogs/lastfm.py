...
async def lastfm_message(ctx: commands.Context, content: str) -> discord.Message: 
 return await ctx.reply(embed=discord.Embed(color=0xff0000, description=f"> <:fmbot:1435343047750586420> {ctx.author.mention}: {content}"))  
...
class Lastfm(commands.Cog):
    ...
    async def on_ready(self): 
     if not clear_caches.is_running():
    ...
    async def lf_customcommand(self, ctx: commands.Context, *, cmd: str):
        ...
        if cmd == "none":   
            if check is None: return await lastfm_message(ctx, f"You don't have a **last.fm** custom command")
    ...
    async def lf_chart(self, ctx: commands.Context, size: str="3x3", period: str="alltime"): 
      ...
      if check is None: return await lastfm_message(ctx, "You don't have a **last.fm** account connected")
      fmuser = check['username']
      if not size in ["3x3", "2x2", "4x5", "20x4"] or not period in ["alltime", "yearly", "monthly"]: raise commands.MissingRequiredArgument("lf chart")
      perio = period.replace('yearly', '12month').replace('monthly', '1month')
      ec = size.replace('x', "*").split('*')      
      limit = int(int(ec[0])*int(ec[1]))
      file = await self.bot.rival.lastfm_chart(username=fmuser, chart_size=size, timeperiod=perio, limit=limit, filename="chart")
      await ctx.reply(content=f"`{ctx.author.name}'s {period} album {size} chart`", file=file)
