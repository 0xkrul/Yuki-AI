...
class Utility(Cog):
    ...
    @hybrid_command(name="avatar", description="See a user's avatar", help="utility", usage="<user>", aliases=["av"])
    async def avatar(self, ctx: Context, *, member: Optional[User] = None):
        member = member or ctx.author
        embed = Embed(color=self.bot.color, title=f"{member.name}'s avatar", url=member.display_avatar.url)
        embed.set_author(name=ctx.author.name, icon_url=ctx.author.display_avatar.url)
        embed.set_image(url=member.display_avatar.url)
        view = View()
        view.add_item(Button(label="Default Avatar", url=member.display_avatar.url))
        if isinstance(member, Member) and member.avatar:
            view.add_item(Button(label="Server Avatar", url=member.avatar.url))
        await ctx.reply(embed=embed, view=view)
