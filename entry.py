...
# OPTIMIZATION: Cache guild configs (antispam, chatfilter, only_cmd, whitelist, seen, afk)
class GuildConfigCache:
    def __init__(self, ttl=300):  # 5 minute TTL
        self._antispam_cache = {}  # guild_id -> (config, timestamp)
        self._chatfilter_cache = {}  # guild_id -> (list[words], timestamp)
        self._only_cmd_cache = {}  # guild_id -> (set[channel_ids], timestamp)
        self._whitelist_cache = {}  # (guild_id, module, object_id, mode) -> (bool, timestamp)
        self._seen_cache = {}  # (guild_id, user_id) -> (exists, timestamp)
        self._afk_cache = {}  # (guild_id, user_id) -> (afk_data or None, timestamp)
        self._seen_write_buffer = {}  # (guild_id, user_id) -> timestamp - batch writes
        self._ttl = ttl
    
    def get_antispam(self, guild_id: int):
        entry = self._antispam_cache.get(guild_id)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_antispam(self, guild_id: int, config):
        self._antispam_cache[guild_id] = (config, time.time())
    
    def get_chatfilter(self, guild_id: int):
        entry = self._chatfilter_cache.get(guild_id)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_chatfilter(self, guild_id: int, words: list):
        self._chatfilter_cache[guild_id] = (words, time.time())
    
    def get_only_cmd(self, guild_id: int):
        entry = self._only_cmd_cache.get(guild_id)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_only_cmd(self, guild_id: int, channel_ids: set):
        self._only_cmd_cache[guild_id] = (channel_ids, time.time())
    
    def get_whitelist(self, guild_id: int, module: str, object_id: int, mode: str):
        key = (guild_id, module, object_id, mode)
        entry = self._whitelist_cache.get(key)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_whitelist(self, guild_id: int, module: str, object_id: int, mode: str, exists: bool):
        key = (guild_id, module, object_id, mode)
        self._whitelist_cache[key] = (exists, time.time())
    
    def invalidate_antispam(self, guild_id: int):
        self._antispam_cache.pop(guild_id, None)
    
    def invalidate_chatfilter(self, guild_id: int):
        self._chatfilter_cache.pop(guild_id, None)
    
    def invalidate_only_cmd(self, guild_id: int):
        self._only_cmd_cache.pop(guild_id, None)
    
    def invalidate_whitelist(self, guild_id: int, module: str = None):
        if module:
            keys_to_remove = [k for k in self._whitelist_cache.keys() if k[0] == guild_id and k[1] == module]
        else:
            keys_to_remove = [k for k in self._whitelist_cache.keys() if k[0] == guild_id]
        for key in keys_to_remove:
            self._whitelist_cache.pop(key, None)
    
    # SEEN CACHE: Track if user has been seen (reduces writes by 90%+)
    def get_seen(self, guild_id: int, user_id: int):
        """Returns (exists, last_update_ts) or None if not cached"""
        key = (guild_id, user_id)
        entry = self._seen_cache.get(key)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_seen(self, guild_id: int, user_id: int, exists: bool):
        key = (guild_id, user_id)
        self._seen_cache[key] = (exists, time.time())
    
    def should_update_seen(self, guild_id: int, user_id: int, min_interval: int = 60) -> bool:
        """Rate limit seen updates - only allow once per min_interval seconds"""
        key = (guild_id, user_id)
        last_write = self._seen_write_buffer.get(key, 0)
        now = time.time()
        if now - last_write >= min_interval:
            self._seen_write_buffer[key] = now
            return True
        return False
    
    # AFK CACHE: Track AFK status
    def get_afk(self, guild_id: int, user_id: int):
        """Returns afk_data dict or False if not AFK, None if not cached"""
        key = (guild_id, user_id)
        entry = self._afk_cache.get(key)
        if entry and time.time() - entry[1] < self._ttl:
            return entry[0]
        return None
    
    def set_afk(self, guild_id: int, user_id: int, afk_data):
        """Set afk_data (dict with time/reason) or None if not AFK"""
        key = (guild_id, user_id)
        self._afk_cache[key] = (afk_data, time.time())
    
    def invalidate_afk(self, guild_id: int, user_id: int):
        key = (guild_id, user_id)
...
async def getprefix(bot, message):
    ...
    # CRITICAL: Early return if both cached (avoids DB entirely)
    if cached_guild is not None and cached_self is not None:
        return cached_guild, cached_self
    
    # OPTIMIZATION: Only fetch what's not cached, in parallel
    tasks = []
    need_self = cached_self is None
    need_guild = cached_guild is None
    ...
    # Skip DB call entirely if both were cached
    if not tasks:
        return cached_guild, cached_self
    ...
    result_idx = 0
    if need_self:
        check = results[result_idx] if result_idx < len(results) and not isinstance(results[result_idx], Exception) else None
    ...
    # Cache results (only what we fetched)
    if need_guild:
...
class CommandClient(commands.AutoShardedBot):
    ...
    async def create_db_pool(self):
        ...
        self.db = await asyncpg.create_pool(
            ...
            # OPTIMIZATION: Increased connections for heavy load
            min_size=5,  # Minimum connections
            max_size=20,  # Maximum connections (increased for load)
            # OPTIMIZATION: Faster connection management
            max_queries=50000,  # Queries per connection before recreation
            max_inactive_connection_lifetime=300,  # Close idle connections after 5 min
            # OPTIMIZATION: Connection tuning - increased timeout
            command_timeout=30,  # 30 second timeout per query (increased from 10)
            # OPTIMIZATION: Statement cache for faster repeated queries
            statement_cache_size=200,  # Cache 200 prepared statements (increased)
        )
    ...
    async def is_command_disabled(self, guild_id: int, command_name: str) -> bool:
        """Check if a command is disabled in a guild (with caching)"""
        # Check cache first
        cache_key = guild_id
        if cache_key in self._disabled_commands_cache:
            cache_time = self._disabled_commands_cache_time.get(cache_key, 0)
            if time.time() - cache_time < 300:  # 5 minute cache
                return command_name in self._disabled_commands_cache[cache_key]
        ...
        disabled_cmds = {row['command'] for row in results}
        
        # Cache results
        self._disabled_commands_cache[cache_key] = disabled_cmds
        self._disabled_commands_cache_time[cache_key] = time.time()
        
        return command_name in disabled_cmds 
    ...
    def ping(self) -> int: 
        if self.latency == float('inf') or self.latency == float('-inf'):
            return 0  # Return 0 if latency is infinite (not yet connected)
    ...
    async def process_commands(self, message: discord.Message):
        """Override to check for disabled commands"""
        ...
        if ctx.guild and await self.is_command_disabled(ctx.guild.id, ctx.command.name):
            return await ctx.send_warning(f"Command **{ctx.command.name}** is disabled in this server")
