...
class Feed:
    ...
    async def for_you(self, count="6", max_cursor="0", min_cursor="0", region="US", raw_data=False) -> list:
        ...
        try:
            ...
            for vid in data:
                if raw_data:
                    ...
                    formatted_video_data = await self.api.video.video_data_formatter(vid)
    ...
    async def trending_categories(self, cursor="0", count="10", region="US", raw=False):
        ...
        """
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api2-19-h2.musical.ly/aweme/v2/category/list/?cursor={cursor}&count={count}&os_api=25&device_type=SM-G973N&ssmix=a&manifest_version_code=2019090808&dpi=320&carrier_region={region}&uoo=0&region={region}&app_name=musical_ly&version_name=13.0.3&is_my_cn=0&ac2=wifi&ac=wifi&app_type=normal&channel=googleplay&build_number=13.0.3&locale=en&sys_region={region}") as trending_categories_request:
                res = await trending_categories_request.json()
        if raw:
            return res
        else:
